...
/**
 * A class that can be used to plot functions in a window. The window has a text
 * field where the user can enter a function. This function can be retrieved
 * using the {@link #getProgram()} method. To plot the function (and display
 * other things) in the window, the programmer can draw on a canvas using
 * <code>draw...()</code> and <code>fill...()</code> methods.
 * <p>
 * To use this class, first, open the window and then draw and call
 * {@link #refreshAndClear(int)} in a loop:
 * 
 * <pre>
 * PlotterWindow window = new PlotterWindow(width, height);
 * window.open();
 * while(window.isOpen()) {
 *     // plot function
...
public class PlotterWindow {
    ...
    private volatile String program = "alpha = i * ((2*PI) / n);\n"
			+ "x = cos(alpha);\n" + "y = sin(alpha);";
    private volatile int iterations = 4;

    /**
     * Create a new plotter window with a plotter area with specified width, and height.
     */
    public PlotterWindow(int width, int height) {
        ...
        panel = new JPanel() {
            public void paintComponent(Graphics g) {
                synchronized(PlotterWindow.this) {
        ...
        splitPane.add(panel);
        splitPane.add(createInputPanel());
        ...
        panel.addComponentListener(new ComponentAdapter() {
            ...
            public void componentResized(ComponentEvent e) {
                PlotterWindow.this.width = toUser(panel.getWidth());
                PlotterWindow.this.height = toUser(panel.getHeight());
            }
        });
    ...
    private JPanel createInputPanel() {
        ...
        panel.add(createProgramPanel(), BorderLayout.CENTER);
        panel.add(createOptionsPanel(), BorderLayout.EAST);
        return panel;
    }
    ...
    private JPanel createOptionsPanel() {
        ...
        JSpinner spinner = new JSpinner(
                new SpinnerNumberModel(iterations, 1, 10000, 1));
        spinner.setAlignmentX(Component.LEFT_ALIGNMENT);
        spinner.setMaximumSize(spinner.getPreferredSize());
        ((DefaultFormatter) ((DefaultEditor) spinner.getEditor()).getTextField().getFormatter())
                .setCommitsOnValidEdit(true);
        spinner.addChangeListener(e -> { iterations = (int) spinner.getValue(); });
        panel.add(spinner);
        panel.add(Box.createVerticalStrut(5));
    ...
    /**
     * Returns the text that is currently in the program input field.
     */
    ...
    /**
     * Returns the number of iterations from the window's input field
     */
